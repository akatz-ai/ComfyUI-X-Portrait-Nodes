...
def visualize_mm(args, name, batch_data, infer_model, nSample, local_image_dir, num_mix=4, preset_output_name=''):
    ...
    # infer_model.appearance_control_model.to(args.local_rank) # ControlNetReferenceOnly
    # infer_model.first_stage_model.to(args.local_rank) # AutoencoderKL
    # infer_model.local_pose_control_model.to(args.local_rank) # ControlNet
    # infer_model.pose_control_model.to(args.local_rank) # ControlNet
    # infer_model.model.to(args.local_rank) # UNet2DConditionModel DiffusionWrapper
    ...
    infer_model.first_stage_model.to("cpu")
    infer_model.cond_stage_model.to(args.local_rank)
    ...
    infer_model.cond_stage_model = infer_model.cond_stage_model.to("cpu")

    with torch.amp.autocast('cuda', enabled=args.use_fp16, dtype=FP16_DTYPE):
        
        infer_model.eval()
        
        infer_model.model.to(args.device)
        infer_model.betas = infer_model.betas.to(args.device)
        infer_model.appearance_control_model.to(args.device)
        infer_model.pose_control_model.to(args.device)
        infer_model.local_pose_control_model.to(args.device)
        ...
        infer_model.betas = infer_model.betas.to(device)
        infer_model.appearance_control_model.to(device)
        infer_model.pose_control_model.to(device)
        infer_model.local_pose_control_model.to(device)
